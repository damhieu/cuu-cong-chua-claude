...
- Platform lơ lửng (v1.5.1): (1) mover đầu màn sunset đâm khối đất trái — kiểm tra tự động (quét toàn range mọi mover × grid solid) chỉ ra overlap 31% ở đầu trái; thu `min` 15.1→16.1 → 0 overlap. (2) 2 platform một chiều (oneway) forest [34,36] & [82,83] đứng im lơ lửng → user muốn di chuyển thật: bỏ khỏi oneways, thêm vào movers dùng class MovingPlatform sẵn có — [34,36] thành mover DỌC (col35, y 11↔13, bắc vực đất-cao-sang-thấp), [82,83] thành mover NGANG (col83, x 82.2↔83.8). Kiểm chứng: quét tự động 0 mover đâm terrain; bot travel đi hết màn 1 tới cửa (0 chết, không kẹt) → cả 2 chỗ qua được. Lưu ý: MovingPlatform là solid 2 chiều (không nhảy xuyên từ dưới như oneway) — chấp nhận vì cả 2 chỗ bắc qua vực, player không đi bên dưới. **Deploy từ 2026-09-02: CHỈ GitHub Pages, không tạo artifact claude.ai nữa (user thấy link github tiện hơn) — xem memory `deploy-game-github-only`.**
- Kiếm rõ lưỡi (v1.4.1): lưỡi cũ ngắn + màu gần trùng giáp + transform lồng nhau → chìm vào thân (chỉ thấy cán). Vẽ lại block "kiếm + tay" trong `Player.draw` với transform phẳng (translate tay → rotate(bladeAng) duy nhất): lưỡi thuôn nhọn dài 38px viền tối #3E4A63 + vạch phản quang, gác chéo lên vai (nghỉ ~-0.95 rad, chạy trễ theo quán tính, trên không ~-0.75), cán nâu + núm chuôi vàng, chắn tay vàng 15px. Đòn chém map sang cách vẽ mới; `atkRect` (hitbox) KHÔNG đổi — verify bằng bot chém giết slime + đi hết màn 1.
- Tranh nền AI Higgsfield (v1.5.0): dùng nano_banana_2 (2 credits/ảnh 2k) sinh 3 backdrop 21:9 + 1 og:image 16:9, có `image_references` = ảnh nền render từ chính game (`Background.draw` offscreen → PNG → media_upload) để bám palette+bố cục. Prompt ghim hex mỗi theme, mô tả style bằng thuật ngữ thị giác thuần (không tên IP). CHỌN: forest/sunset ảnh CÓ tháp+tia (hasBakedLight:true → tắt guiding-light động + god rays code khi ảnh ready); castle ảnh KHÔNG có lâu đài (bgReplaceN:1 giữ skyline lâu đài code-vẽ; hasBakedLight:false → giữ ánh sáng dẫn đường động). Tổng chi: **22 credits**. Tích hợp `background.js`: theme thêm bgSrc/bgF/bgReplaceN/hasBakedLight/bgYOff; `_loadBg` preload async có cache; `draw` khi `bgImg.__ready` → `_drawBgImage` (1 drawImage đủ rộng phủ tầm cuộn, không tile, parallax f theo bgF) thay sky+sun/moon+stars+clouds + bỏ bgReplaceN lớp xa; **fallback**: ảnh lỗi/chưa tải → `_drawSky` + renderer cũ nguyên vẹn (đã test bằng ép `__ready=false`). Ảnh nén `sips` JPEG q80 ~2048px → 100-190KB/ảnh, tổng assets 552KB. render 0.29ms (nhẹ hơn trước). og:image + meta OG/twitter trong index.html (URL tuyệt đối GitHub Pages). Bản artifact: script build thay `assets/*.jpg` → data-URI base64 (ASCII-safe), bundle 0.63MB.
- Public deploy (v1.4.0, 02/09/2026): tạo repo GitHub công khai `damhieu/cuu-cong-chua-claude` (dùng `gh` đã đăng nhập sẵn trên máy, không tạo tài khoản mới), push toàn bộ mã nguồn (không build/transform gì — copy thẳng), bật GitHub Pages (`gh api repos/damhieu/cuu-cong-chua-claude/pages -X POST -f "source[branch]=main" -f "source[path]=/"`). URL công khai, không cần đăng nhập gì: **https://damhieu.github.io/cuu-cong-chua-claude/**. Đây là kênh chia sẻ chính cho bạn bè — bản claude.ai artifact (37f6d055…) vẫn giữ song song cho việc riêng tư/xem trước.
  **Quy trình cập nhật khi sửa game (nhớ làm đủ, đừng quên kênh nào):**
  1. Sửa code trong `js/`/`index.html`, bump `GAME_INFO` trong `js/util.js`.
  2. Test qua server cục bộ (`.claude/launch.json`, port 8642).
  3. `git add -A && GIT_AUTHOR_NAME="Đàm Mạnh Hiếu" GIT_AUTHOR_EMAIL="dammanhhieu@gmail.com" GIT_COMMITTER_NAME="Đàm Mạnh Hiếu" GIT_COMMITTER_EMAIL="dammanhhieu@gmail.com" git commit -m "..."` rồi `git push` (repo không có global git identity — dùng biến môi trường per-commit, KHÔNG chạy `git config`, đúng luật an toàn của phiên). GitHub Pages tự rebuild trong ~30–60s.
  4. (Tuỳ chọn) build lại bundle 1-file ASCII-safe cho claude.ai artifact (script Python gộp index.html + js/*, escape unicode) và publish lại với cùng `url` để giữ nguyên link claude.ai.
